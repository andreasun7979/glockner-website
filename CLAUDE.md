...
- Fonts: Montserrat (headings via CSS rule), DM Sans (body), **Fraunces** (Lux3 serif display, used via `.font-serif-display`) — all loaded **self-hosted via `next/font/google`** in `layout.tsx` (imported as `DM_Sans`, `Fraunces`, `Montserrat`, each exposing a `--font-*` CSS variable; no render-blocking request to Google's CDN). This **replaced** the earlier `<link>`-to-Google-Fonts approach and **closes the DSGVO Google-Fonts gap** (see DSGVO notes). When adding weights/subsets, edit the `next/font` import options, not a CDN URL.
...
- `/ki-schulungen` — **Training** landing page: AI-OS setup + Claude Code + agent system + the **Schulungen** block (moved here from lux3: Agentic AI / Claude Code / AI im Unternehmen) + package "ab €1.900/Mo + Setup".
...
- `/foerderung` — **KI-Förderseite für KMU** (canonical `/foerderung`, indexable, in sitemap @ priority 0.9, middleware passthrough). KMU.DIGITAL-Beratung (beide Module: Status-/Potenzialanalyse + Strategieberatung) als Baustein-Block + FAQ, plus die „30 % Glockner Förderrabatt"-Story. Added 2026-06.
...
- **Förder-Story (umgestellt 2026-06, mehrere Commits):** die Seiten werben jetzt einheitlich mit einem **„30 % Glockner Förderrabatt"** (z.B. Preis-Tiles `-30%: €350 / €1.400`) — **nicht** mehr mit „bis zu 80 % WKO-Förderquote" oder dem alten Drei-Tier-Paket `€100 / €1.000 / bis €6.000`. Echte WKO/CeCC-Qualifikations-Credentials (zertifizierter Berater) bleiben; nur die *Förderquoten*-Claims wurden zum Rabatt. Die `/leistungen/*`-Altseiten (noindex) tragen teils noch die alte Sprache. **Wenn du Preis-/Förderzahlen änderst, lies zuerst die aktuelle Tile-Struktur auf `/digital/ecommerce` und repliziere die identische Struktur über alle drei Digital-Seiten.**
...
- **GA4** (`G-DV7B3ZGSFY`) is loaded by `src/components/Analytics.tsx` — a client component that injects `gtag.js` **only after** Cookiebot reports `statistics` consent (`CookiebotOnAccept` / `CookiebotOnConsentReady`; return visits via `window.Cookiebot.consent.statistics`). Fail-safe: if Cookiebot can't load, no consent event fires → GA never loads.
  - **Dedicated GA4 property (2026-06-16):** `G-DV7B3ZGSFY` is the web stream (`glockner.ai Web`, Stream-ID `15083264422`) of the **own `glockner.ai` property** (`properties/...`, account *Glockner Digital* `318922935`, TZ Europe/Vienna, EUR). It **replaced** the previous `G-LJ8G8DE1F0`, which was a stream under the **`glocknerdigital.at` property** (`446963450`) and mixed both brands' traffic. Historical pre-2026-06-16 glockner.ai data therefore lives in the old `glocknerdigital.at` property; new data flows into the dedicated `glockner.ai` property. To swap again, edit `GA_ID` in `Analytics.tsx` + the Mess-ID in `/datenschutz`.
...
- Verified end-to-end: before consent → no `gtag.js`, no `_ga`, 0 `/collect`; after "Alle zulassen" → `page_view` hits `region1.analytics.google.com/g/collect?tid=G-DV7B3ZGSFY`, `_ga` set, choice stored in the `CookieConsent` cookie.
...
- **Google-Fonts DSGVO gap — RESOLVED.** Fonts are now self-hosted at build time via `next/font/google` in `layout.tsx` (no runtime request to Google's CDN), which removes the dynamic-embedding risk flagged in LG München I, 3 O 17493/20. There is no longer a `<link>` to `fonts.googleapis.com` and no `public/fonts/` directory (next/font manages the files).
